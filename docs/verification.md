...
Corrected application revision **`682090227c5d099f337315c12ce034b27b0e526f`** deployed successfully at **https://kottabos.onrender.com** after a 42.7-second rollout begun at 16:53:09 MDT. Render confirmed that exact commit, Node 24.21.0, clean lockfile install/build and production startup. The next public SDK run passed capacity/recovery and completed the full 45-second round with twelve survivors and matching scores, but timed out awaiting the standalone SDK host's explicit leave: its default automatic retry restarted after an abnormal socket close on the hosted route. The application already disables retry before explicit leave. The test harness now does so too, with recovery exercised explicitly; this is a test-only correction. All temporary seats from the incomplete run were released.

The corrected **public twelve-SDK-client acceptance passed** in **91.9 seconds**. All twelve completed the normal **45,000 ms** round and survived the fully observed two-square finale, with identical results/scores. Raw HTTP and SDK checks confirmed clear thirteenth-player rejection both while connected and while a seat was reserved. Identity recovery with rotated credentials, all-player movement, replay, host leave/transfer, replacement and the next twelve-player round start passed. Cleanup confirmed the temporary party no longer existed. From this computer's internet route, 528 game pings observed **25 ms median / 28 ms p95**; maximum sampled simulation callback was **1.99 ms**. These are one-party observations, not twelve phones or a multi-party benchmark.

The **public twelve-player browser case passed** in **1.4 minutes**, using the deployed HTTPS page in Chrome with phone emulation, Edge, and ten SDK clients. It verified 12/12 roster and painted characters, stable numbers through replacement, portrait/narrow/landscape layouts with no overlap from the lobby banner, matching results, replay, refresh at capacity and host transfer. No uncaught browser JavaScript errors were reported. It was run with `PLAYTEST_URL=https://kottabos.onrender.com` and `playwright test --config playwright.remote.config.js tests/browser/twelve-player.spec.js`; the remote configuration now includes that case alongside the basic Chrome/Edge smoke test.

Test players left, isolated browsers closed, and no temporary local test servers remain. The user's open game page was refreshed and shows **2–12 PLAYERS**. The existing Free service remains available and may idle normally; no keep-awake task exists. Subsequent test/documentation-only commits do not change the deployed application revision above. Twelve is recorded in contributor guidance as the shared foundation for future games; this completes the owner's requested stopping point for today's development.
