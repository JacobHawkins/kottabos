...
The arena is seven by seven 64-pixel tiles: 448 by 448 pixels. Up to twelve players receive distinct colors and stable palette-derived numbers (01–12). Twelve tile-centered spawns form a symmetric inset perimeter, retaining the original four corners and adding eight positions at least 64 pixels apart. Nobody starts on a possible final square. A countdown captures the connected participants; a new arrival after that capture spectates until a later round.

The lobby draws occupied color slots in four columns and three rows, so remaining characters/numbers do not reshuffle when someone leaves. During play, numbered characters replace remote name plates; the full roster retains names and scores. The local character has a white ring, YOU label and higher draw depth. Ready/start/replay controls precede the compact roster, and results awards wrap on phones. These are presentation choices; the server alone decides position and elimination, and players can still share a tile.

Normal rounds last at most 45 seconds of simulation time. Each seeded schedule chooses one of twelve adjacent pairs in the central 3×3 area. Horizontal/vertical orientations and positions vary symmetrically around the corner spawns. Manhattan distance to the nearer of the two squares orders the earlier removal waves; equal-distance tiles are shuffled. Every surviving tile retains an orthogonal path inward. Both final squares stay safe for 2.5 seconds, then a separate seeded random stream chooses one for the ordinary 1.8-second warning and removal. The survivor remains for the final 2.5 seconds. The earlier pattern is built around the unordered pair, so it does not favor or reveal the survivor. The full schedule and random choice stay server-private.

For an ordinary 45-second round, the pair is fully exposed at 38.2 seconds, the final warning begins at 40.7 seconds, and the chosen square falls at 42.5 seconds. A round can still end earlier if only one participant remains. Accelerated test timings retain the full warning and pair pause; a three-second round ends on its two surviving squares before the final-choice stage, rather than compressing it into an unavoidable fall.

The first warning begins after four seconds; each disappearing tile warns for 1.8 seconds. A 200-seed test checks all twelve pair locations, both endpoints surviving, connected remaining floor and a path to surviving floor within the warning's movement allowance at ordinary timings. Tile states are `0` safe, `1` warning and `2` gone. A player's center entering a gone tile eliminates them; placeholder character radius is visual only. There is no jumping, pushing or player-to-player collision. The original center-only design is preserved as history in journal 001; it is no longer the current rule.
